...
class Stores extends Admin_Controller
{
    ...
    */
	public function remove()
	{
		...
		$response = array();
		if ($store_id) {
			// check whether store exists in product
			if ($this->db->where(['store_id' => $store_id])->from("products")->count_all_results() > 0) {
				$response['success'] = false;
				$response['messages'] = "Store exists in the products";
			} else {
