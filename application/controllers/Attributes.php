...
class Attributes extends Admin_Controller
{
	...
	*/
	public function remove()
	{
		...
		$response = array();
		if ($attribute_id) {
			// emptied the attribute values
			while ($this->db->where(['attribute_parent_id' => $attribute_id])->from('attribute_value')->count_all_results() > 0) {
				$this->db->where(['attribute_parent_id' => $attribute_id])->delete('attribute_value');
			}
